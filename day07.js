...
function parse (line) {
  ...
  const rgxp = /(.*) \((.*)\)/g;
  const match = rgxp.exec(parts[0]);

  let discs = [];
  if (parts.length > 1) {
    discs = parts[1].split(',').map((d) => {
      return d.trim()
    });
  }
...
function findRoot (lines) {
  ...
  lines.forEach((obj) => {
    ...
    if (Array.isArray(obj.discs)) {
      children.push(...obj.discs);
    }
  });

  let root = names.filter((name) => {
    return children.indexOf(name) === -1;
  });
...
console.log(findRoot(parseLines(lines)));


function buildTree (lines) {
  const objects = {};
  const rootName = findRoot(lines);
  ...
  lines.forEach((o) => {
    if (Array.isArray(o.discs)) {
      o.discs = o.discs.map((n) => {
        return objects[n]
      });
    }
  });
...
function sum (prog) {
  ...
  if (Array.isArray(prog.discs)) {
    for (let i = 0; i < prog.discs.length; i++) {
      total += sum(prog.discs[i]);
    }
  }

  return total + prog.weight;
}

const root = buildTree(parseLines(lines));
const totals = {};

root.discs.forEach((p) => {
  let total = sum(p);
